...
def read_tag():
    ...
    if status != reader.MI_OK:
        return False
    print(uid)
    return True if uid == [163, 89, 57, 247, 52] else False
...
def read_measurements():
    ...
    return temperature, humidity, evaporator_temperature

def send_measurements(temperature, humidity, evaporator_temperature):
        payload = {
        ...
        "is_admin": current_state.get("is_admin")
        }
        try:
    ...
    try:
        while True:
            ...
            if now_mono - last_measurements_read > SENSOR_READ_INTERVAL:
                current_state["is_admin"] = read_tag()
                temperature, humidity, evaporator_temperature = read_measurements()
                last_measurements_read = now_mono
                print(f"temperature: {temperature}\thumidity: {humidity}\tevaporator_temperature:{evaporator_temperature}\tis_admin:{current_state['is_admin']}")
                print(current_state)
            ...
            if temperature is None or evaporator_temperature is None:
                ...
                if (now_mono - last_upload) >= UPLOAD_INTERVAL:
                    send_measurements(temperature, humidity, evaporator_temperature)
            ...
            # Upload sensors
            if (now_mono - last_upload) >= UPLOAD_INTERVAL:
                if send_measurements(temperature, humidity, evaporator_temperature) == 0:
                    last_upload = now_mono
                    connection_fault_triggered = False
                    ever_connected = True
                else:
